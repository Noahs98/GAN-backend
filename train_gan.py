        ...
        # for epoch in range(max([int(args.gen_pretrain_num_epochs), int(args.disc_pretrain_num_epochs)])):
            for i, (images, captions, lengths, wrong_captions, wrong_lengths) in enumerate(data_loader):
                ...
                if (i + 1) % args.log_step == 0:
                    print('Epoch [%d], Step [%d], Disc Loss: %.4f, Gen Loss: %.4f'
                          % (epoch + 1, i + 1, loss_disc, loss_gen))
                if (i + 1) % 500 == 0:
                    torch.save(discriminator.state_dict(), os.path.join(args.model_path,
                                                        'pretrained-discriminator-%d-%d.pkl' % (int(epoch) + 1, i + 1)))
                    torch.save(generator.state_dict(), os.path.join(args.model_path,
                                                            'pretrained-generator-%d-%d.pkl' % (int(epoch) + 1, i + 1)))
	    ...
	    # for t in tqdm(range(2, Tmax, 4)):
            ...
            # for t in range(2, 4):
                ...
                for batch in range(int(captions.size(0))):
                    for b_t in range(Tmax):
                        if gen_samples[batch, b_t].cpu().data.numpy() == 2: # <end>
            ...
            for batch in range(int(captions.size(0))):
                for b_t in range(20):
                    ...
                    if sampled_ids[batch, b_t].cpu().data.numpy() == 2: # <end>
                        sampled_lengths.append(b_t+1)
                        break
                    elif b_t == 20-1:
            ...
            if (i + 1) % args.log_step == 0: # jm: saving at the last iteration instead
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_gen_path', type=str, default='./models/pretrained-generator-15-4000.pkl')
    parser.add_argument('--pretrained_disc_path', type=str, default='./models/pretrained-discriminator-15-4000.pkl')
